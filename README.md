...
> **Примечание по конфигурации:** Файл `docker-compose.yml` поддерживает переопределение переменных. Если вы хотите использовать свои порты, названия БД или пароли, вы можете передать их через поддержку переменных окружения в консоли (например, `POSTGRES_PORT=5433 docker compose up -d`) или просто создать файл `.env` в корне проекта. По умолчанию используются настройки: порт `5432`, база `petstore`, логин/пароль `user`/`password`.
